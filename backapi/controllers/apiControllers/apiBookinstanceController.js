...
exports.bookinstance_list = (req, res, next) => {
    Bookinstance.find()
        ...
        .exec((err, list_bookinstance) => {
            ...
            res.send({title: 'Book Instance List', bookinstance_list: list_bookinstance});
        })
};
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            bookinstance.save((err) => {
                ...
                res.send({message:"Data from form is valid ,save success!", url:bookinstance.url});
            })
        }
    }
];
...
exports.bookinstance_delete_get = function(req, res, next) {
    ...
    .exec(function (err, bookinstance) {
        ...
        // Successful, so render.
        res.send({ title: 'Delete BookInstance', bookinstance:  bookinstance});
    })
        ...
        if (!errors.isEmpty()) {
            ...
            Book.find({},'title')
                .exec(function (err, books) {
                    if (err) { return next(err); }
                    // Successful, so render.
                    res.send({ title: 'Update BookInstance', book_list : books, selected_book : bookinstance.book._id , errors: errors.array(), bookinstance:bookinstance });
            });
            return;
        }
        else {
            ...
            BookInstance.findByIdAndUpdate(req.params.id, bookinstance, {}, function (err,thebookinstance) {
                if (err) { return next(err); }
                   ...
                   res.send("data from form is valid ,update sucess!");
                });
        }
    }
];
